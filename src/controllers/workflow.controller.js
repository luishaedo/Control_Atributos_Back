...
export function WorkflowController(prisma) {
  ...
  return {
    ...
    closeCampaign: async (req, res) => {
      ...
      const consolidateStages = await prisma.skuStage.findMany({
        where: { campaniaId, stage: "consolidate" },
        select: { sku: true },
      });
      const consolidateSkus = consolidateStages.map((row) => row.sku);
      const unknowns = consolidateSkus.length
        ? await prisma.unknownSku.findMany({
            where: { campaniaId, sku: { in: consolidateSkus } },
          })
        : [];

      const invalidUnknowns = [];
      if (unknowns.length) {
        const categories = Array.from(
          new Set(unknowns.map((u) => normalizeCode(u.categoria_cod)).filter(Boolean))
        );
        const types = Array.from(
          new Set(unknowns.map((u) => normalizeCode(u.tipo_cod)).filter(Boolean))
        );
        const classifications = Array.from(
          new Set(unknowns.map((u) => normalizeCode(u.clasif_cod)).filter(Boolean))
        );
        const [dicCats, dicTypes, dicClasif] = await Promise.all([
          prisma.dicCategoria.findMany({ where: { cod: { in: categories } } }),
          prisma.dicTipo.findMany({ where: { cod: { in: types } } }),
          prisma.dicClasif.findMany({ where: { cod: { in: classifications } } }),
        ]);
        ...
        for (const unknown of unknowns) {
          ...
          const clasif_cod = normalizeCode(unknown.clasif_cod);
          if (!categoria_cod || !tipo_cod || !clasif_cod) {
            invalidUnknowns.push({ sku: unknown.sku, reason: "missing_fields" });
            continue;
          }
          if (!dicCatSet.has(categoria_cod)) {
            invalidUnknowns.push({ sku: unknown.sku, reason: "invalid_categoria" });
            continue;
          }
          if (!dicTypeSet.has(tipo_cod)) {
            invalidUnknowns.push({ sku: unknown.sku, reason: "invalid_tipo" });
            continue;
          }
          if (!dicClasifSet.has(clasif_cod)) {
            invalidUnknowns.push({ sku: unknown.sku, reason: "invalid_clasif" });
            continue;
          }
        }
      }

      if (invalidUnknowns.length) {
        return res.status(400).json({
          error: "Unknown SKUs inválidos para consolidar",
          invalidItems: invalidUnknowns,
        });
      }
        ...
        await prisma.$transaction(async (tx) => {
          for (const unknown of unknowns) {
            ...
            await tx.unknownSku.update({
              ...
              data: { status: "consolidated", updatedAt: new Date() },
            });
          }
        });
      }
      ...
      res.json({
        ok: true,
        applied: count,
        consolidatedUnknowns: unknowns.length,
        summary,
      });
    },
  };
